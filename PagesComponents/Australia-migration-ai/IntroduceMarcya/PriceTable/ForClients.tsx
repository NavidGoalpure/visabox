...
import { MdDiamond, MdOutlineMoneyOff } from 'react-icons/md';
      ...
      <Grid>
        ...
        >
          <CardTitle>
            ...
            <h2>{t(LanguageKeys.FreePlan_Title)}</h2>
          </CardTitle>
          <Price>
            <h4>{t(LanguageKeys.FreePlan_Price)}</h4>
            <h5>{t(LanguageKeys.FreePlan_ABV)}</h5>
          </Price>
          <Option>
            <ul>
          ...
          <PrimaryButton
            ...
            onClick={() => router.push('/australia-migration-ai/chat')}
          >
            {t(LanguageKeys.FreePlan_Button)}
          </PrimaryButton>
        </MyCard>
...
const MyCard = styled.div<{ $locale?: Languages }>`
  ...
  overflow: hidden;
  min-height: ${({ $locale }) =>
    $locale === Languages.fa ? '28rem' : '25rem'};
  @media ${deviceMin.tabletS} {
    min-height: ${({ $locale }) =>
      $locale === Languages.fa ? '29rem' : '27rem'};
  }
...
const StyledIcon = css`
  ...
  padding: 16px;
`;
const StyledFreeIcon = styled(MdOutlineMoneyOff)`
  ${StyledIcon}
  color: var(--color-secondary4);
  // background: linear-gradient(-45deg, #ffffcc, #ffffff);
`;
